...
def plot_rti(data_spc, utc, normFactor, metadata, channel_names, 
    ...
    '''
    Plot RTI graph based on spectral data with customizable limits, showing all data without percentile filtering.
    Uses an enhanced purple color for the highest values to make them more distinguishable from red.
    ...
    gs = fig.add_gridspec(n_channels, 1, height_ratios=[1] * n_channels, 
                         ...
                         hspace=0.4)  # Increased hspace for more space between subplots
    ...
    # Define more intense purple endpoint (rich, vibrant purple)
    # RGB values: R=0.6, G=0.0, B=0.8 (more vibrant purple with higher blue component)
    target_purple = np.array([0.6, 0.0, 0.8, 1.0])  # Increased blue component for more vivid purple
    ...
    for ch in range(n_channels):
        ...
        # Create the pcolormesh with full data range
        mesh = ax.pcolormesh(
            time_axis,
...
plot_rti(data_spc, utc, 
        ...
        channel_names, 
        power_min=15,
        power_max=30,  # Límites de potencia personalizados
        height_min=60, 
        height_max=200)  # Rango de alturas personalizado
